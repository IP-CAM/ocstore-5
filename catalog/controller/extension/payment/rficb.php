...
class ControllerExtensionPaymentRFICB extends Controller {
	public function index() {
		$this->language->load('extension/payment/rficb');
		...
		$data['comission'] = $this->config->get('rficb_commission');
		$data['pay_type'] = $this->config->get('rficb_payment_type');
		$data['pay_spg'] = $this->config->get('rficb_payment_spg');
		$data['pay_wm'] = $this->config->get('rficb_payment_wm');
		$data['pay_ym'] = $this->config->get('rficb_payment_ym');
		$data['pay_mc'] = $this->config->get('rficb_payment_mc');
		$data['pay_qiwi'] = $this->config->get('rficb_payment_qiwi');
		...
		return $this->load->view('extension/payment/rficb.tpl', $data);
	}
   ...
   // echo "test2-".$ch ;
	...
	/*		$data['continue'] = $this->url->link('checkout/success');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/extension/payment/rficb_failure.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/extension/payment/rficb_failure.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('extension/payment/rficb_failure.tpl', $data));
			}
		}
	}    
